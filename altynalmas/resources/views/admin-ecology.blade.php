...
@section('content')
    <div class="container text-xl-center admin-text">
        Здесь Вы можете добавить, изменить или удалить информацию для страницы "Экология", а так же выбрать, будет ли информация отображаться на сайте.
    </div>

    <div class="block">
        <div class="block-content block-content-narrow">
            <form class="form-horizontal" action="{{ route('admin-ecology.store')}}" method="post" enctype="multipart/form-data">
                @csrf
        ...
        @else
            ...
            <table class="table table-striped mb-5">
                <tbody>
                    ...
                    @foreach($ecology as $eachEcology)
                        <tr>
                            <td>
                                {{ $eachEcology->title_ru }}
                            </td>
                            <td>
                                @if ($eachEcology->is_active == 1)
                            ...
                            <td>
                                <a href="{{ route('admin-ecology.edit', $eachEcology->id )}}" class="btn btn-info"> Редактировать </a>
                            </td>
                            <td>
                                <form action="{{ route('admin-ecology.destroy', $eachEcology->id )}}" method="post">
                                    @csrf
                                    {{method_field('DELETE')}}
