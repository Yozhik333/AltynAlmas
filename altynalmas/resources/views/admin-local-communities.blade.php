...
@section('content')
    <div class="container text-xl-center admin-text">
        Здесь Вы можете добавить, изменить или удалить информацию для страницы "Местные сообщества", а так же выбрать, будет ли информация отображаться на сайте.
    </div>

    <div class="block">
        <div class="block-content block-content-narrow">
            <form class="form-horizontal" action="{{ route('admin-local-communities.store')}}" method="post" enctype="multipart/form-data">
                @csrf
    ...
    @isset($localCommunities)
        @if (count($localCommunities) == 0)
            <div class="container text-xl-center admin-text">
        ...
        @else
            ...
            <table class="table table-striped mb-5">
                <tbody>
                    ...
                    @foreach($localCommunities as $localCommunity)
                        <tr>
                            <td>
                                {{ $localCommunity->title_ru }}
                            </td>
                            <td>
                            ...
                            <td>
                                <a href="{{ route('admin-local-communities.edit', $localCommunity->id )}}" class="btn btn-info"> Редактировать </a>
                            </td>
                            <td>
                                <form action="{{ route('admin-local-communities.destroy', $localCommunity->id )}}" method="post">
                                    @csrf
                                    {{method_field('DELETE')}}
